...
class textCNN(nn.Module):
    def __init__(self, Vocab, Dim, Cla, Ci=1, Knum=200, Ks=[3,4,5], dropout = 0.0):
        ...
        self.fc0 = nn.Linear(48, Cla)
        self.fc1 = nn.Linear(47, Cla)  ##全连接层
        self.fc2 = nn.Linear(46, Cla)
        # self.initNetParams()
        self.Knum = Knum
    ...
    def forward(self, x):
        ...
        x = [F.tanh(conv(x)).squeeze(3) for conv in self.convs]  # len(Ks)*(N,Knum,W)

        x = [F.max_pool1d(line, line.size(2)).squeeze(2) for line in x]  # len(Ks)*(N,Knum)
        y = torch.stack((x[0], x[1], x[2]), 0)
